...
func (sch *Scheduler) runTask(t task) {
	...
	log.Debug("Scheduler run process", zap.String("func", t.process.GetName()), zap.String("type", string(t.runType)))
}
...
func runByInterval(ctx context.Context, process Process, interval time.Duration) {
	if interval == 0 {
		log.Error("Scheduler: interval is zero", zap.String("process", process.GetName()))
		return
	}
...
func runByPeriod(ctx context.Context, process Process, period time.Duration) {
	if period == 0 {
		log.Error("Scheduler: period is zero", zap.String("process", process.GetName()))
		return
	}
